...
namespace osu.Game.Rulesets.Mix
{
    public class MixDifficultyCalculator : DifficultyCalculator<MixHitObject>
    {
        ...
        protected bool CalculateStrainValues()
        {
            ...
            using (List<MixHitObjectDifficulty>.Enumerator hitObjectsEnumerator = DifficultyHitObjects.GetEnumerator())
            {

                if (!hitObjectsEnumerator.MoveNext()) return false;

                MixHitObjectDifficulty current = hitObjectsEnumerator.Current;
        ...
        protected double CalculateDifficulty(DifficultyType type)
        {
            ...
            MixHitObjectDifficulty previousHitObject = null;
            foreach (MixHitObjectDifficulty hitObject in DifficultyHitObjects)
            {
                while (hitObject.BaseHitObject.StartTime > intervalEndTime)
                {
                    ...
                    {
                        double decay = Math.Pow(MixHitObjectDifficulty.DECAY_BASE[(int)type], (intervalEndTime - previousHitObject.BaseHitObject.StartTime) / 1000);
