...
{
    internal class ClassicBeatmapProcessor : BeatmapProcessor
    {
        public ClassicBeatmapProcessor(IBeatmap beatmap)
        : base(beatmap)
        {
        }
        ...
        {
            applyStacking((Beatmap<ClassicHitObject>)Beatmap);
            base.PostProcess();
        }
        ...
        private void applyStacking(Beatmap<ClassicHitObject> beatmap)
        {
            ...
            for (int i = beatmap.HitObjects.Count - 1; i >= 0; i--)
            {
                ...
                {
                    ClassicHitObject stackBaseObject = beatmap.HitObjects[stackBaseIndex];
                    if (stackBaseObject is Spinner) break;

                    ClassicHitObject objectN = beatmap.HitObjects[n];
            ...
            for (int i = extendedEndIndex; i > 0; i--)
            {
                ...
                ClassicHitObject objectI = beatmap.HitObjects[i];
                    ...
                    while (--n >= 0)
                    {
                        ...
                        if (objectN is Slider && Vector2Extensions.Distance(objectN.EndPosition, objectI.Position) < stack_distance)
                        {
                            ...
                            for (int j = n + 1; j <= i; j++)
                            {
                                ...
                                ClassicHitObject objectJ = beatmap.HitObjects[j];
