...
namespace osu.Game.Rulesets.Circles.OsuDifficulty.Preprocessing
{
    ...
    public class OsuDifficultyBeatmap : IEnumerable<OsuDifficultyHitObject>
    {
        ...
        /// </summary>
        public OsuDifficultyBeatmap(List<ClassicHitObject> objects, double timeRate)
        {
        ...
        private IEnumerator<OsuDifficultyHitObject> createDifficultyObjectEnumerator(List<ClassicHitObject> objects, double timeRate)
        {
            // We will process OsuHitObjects in groups of three to form a triangle, so we can calculate an angle for each object.
            ClassicHitObject[] triangle = new ClassicHitObject[3];
