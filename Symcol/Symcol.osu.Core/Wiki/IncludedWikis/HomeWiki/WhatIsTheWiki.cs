using Symcol.osu.Core.Wiki.Sections;

namespace Symcol.osu.Core.Wiki.IncludedWikis.HomeWiki
{
    public class WhatIsTheWiki : WikiSection
    {
        public override string Title => "What is the wiki?";

        public override string Overview => "This is the Symcol ingame wiki, featuring content from mods and rulesets that have wikis added in. "
                                           + "You can use it to browse information about mods and rulesets to learn how to use them or how to play, "
                                           + "get started by selecting a wiki in the index near the top left.";
    }
}
