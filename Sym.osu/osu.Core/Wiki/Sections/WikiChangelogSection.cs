...
namespace osu.Core.Wiki.Sections
{
    public abstract class WikiChangelogSection : WikiSection
    {
        ...
        private void load(Storage storage)
        {
            ...
            {
                Logger.Log("Could not create / update current version file in changelogStorage!", LoggingTarget.Database, LogLevel.Error);
            }
            ...
            {
                Logger.Log("Error reading saved versions (if you have any, if not then that is the problem)", LoggingTarget.Database, LogLevel.Error);
            }
        }
    }
}
